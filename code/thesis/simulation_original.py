...
class Midge():
    ...
    def move(self):
        ...
        if self.x < 0 or self.x > parameters.ECOSYSTEM["Width"] or self.y < 0 or self.y > self.height: 

            if self.y < 0:
                self.y = parameters.ECOSYSTEM["Height"]

            if self.y > parameters.ECOSYSTEM["Height"]:
                self.y = 0
...
class Mite():
    ...
    def move(self):
        ...
        # #Setting the borders, round world
        if self.x < 0 or self.x > parameters.ECOSYSTEM["Width"] or self.y < 0 or self.y > parameters.ECOSYSTEM["Height"]: 
